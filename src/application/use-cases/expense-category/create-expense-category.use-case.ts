...
import { EXPENSE_CATEGORY_REPOSITORY, IExpenseCategoryRepository } from "@domain/interfaces/repositories";
...
export class CreateExpenseCategoryUseCase implements IBaseUseCase<CreateExpenseCategoryRequestDto, GetExpenseCategoryResponseDto> {
  ...
  constructor(
    @Inject(EXPENSE_CATEGORY_REPOSITORY) private readonly categoryExpenseRepository: IExpenseCategoryRepository,
  ...
  async execute(data: CreateExpenseCategoryRequestDto): Promise<GetExpenseCategoryResponseDto> {
    ...
    const existingCategory = await this.categoryExpenseRepository.findByName(data.name);

    if (existingCategory) {
      this.loggerService.warn(`Expense category with name "${data.name}" already exists for company ID ${companyId}`);
