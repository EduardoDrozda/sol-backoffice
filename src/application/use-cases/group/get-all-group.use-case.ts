...
@Injectable()
export class GetAllGroupUseCase implements IBaseUseCase<GetPaginationBaseDto, BaseResponseWithPaginationDto<GetGroupResponseDto>> {
  constructor(
    @Inject(GROUP_REPOSITORY) private readonly groupRepository: IGroupRepository,
    private readonly loggerService: LoggerService
  ) { }

  async execute(data: GetPaginationBaseDto): Promise<BaseResponseWithPaginationDto<GetGroupResponseDto>> {
    ...
    return PaginationHelper
      .paginate<GetGroupResponseDto>(groups, page, limit);
  }
} 