...
export class UserService {
  ...
  resendConfirmationEmail(id: string) {
    return this
      .http
      .post<Record<string, unknown>, Record<string, unknown>>(`${this.baseUrl}/${id}/resend-confirmation-email`, {})
      .pipe(
        map((response) => response.result)
      );
  }
  ...
  resetUserPassword(id: string) {
    return this
      .http
      .post<Record<string, unknown>, Record<string, unknown>>(`${this.baseUrl}/${id}/reset-password`, {})
      .pipe(
        map((response) => response.result)
      );
  }
