...
export class AuthService {
  ...
  signIn(credential: CredentialModel) {
    return this.http.post<CredentialModel, SignInResponseModel>('/v1/login', credential);
  }

  checkToken() {
    return this.http.get<UserModel>('/v1/check');
  }

  confirmEmailWithPassword(data: { token: string; password: string; confirmPassword: string }) {
    return this.http.post<Record<string, unknown>, Record<string, unknown>>('/v1/users/confirm-email', data);
  }

  requestPasswordReset(data: ResetPasswordRequestModel) {
    return this.http.post<ResetPasswordRequestModel, ResetPasswordResponseModel>('/v1/users/forgot-password', data);
  }

  resetPassword(data: ResetPasswordConfirmModel) {
    return this.http.patch<ResetPasswordConfirmModel, ResetPasswordResponseModel>('/v1/users/reset-password', data);
  }
